...
export const ChartBarInteractive = React.memo(function ChartBarInteractive({
  ...
  const getDateFromMouseX = React.useCallback(
    ...
    <Card className="py-0 w-full h-full flex flex-col overflow-hidden">
      <CardContent className="px-2 sm:p-6 w-full h-full flex flex-col min-h-0 flex-1">
        ...
        <div className="flex justify-end w-full pt-4 flex-shrink-0">
          <div
            className="inline-flex h-6 items-stretch rounded-[4px] overflow-hidden border border-slate-300 text-xs leading-none"
            role="status"
            aria-label={
              selectedDataPoint
                ? `Selected data point: ${selectedDataPoint.date}, ${activeChart === "desktop" ? "Desktop" : "Mobile"} views: ${activeChart === "desktop" ? selectedDataPoint.desktop : selectedDataPoint.mobile}`
                : "No data point selected"
            }
          >
            <div
              className="flex items-center justify-center bg-emerald-600 px-3 text-white font-semibold"
              aria-label="Date"
            >
              {selectedDataPoint ? selectedDataPoint.date : "----------"}
            </div>
            <div
              className="flex min-w-[48px] items-center justify-center bg-white px-3 text-black font-semibold"
              aria-label={activeChart === "desktop" ? "Desktop views" : "Mobile views"}
            >
  ...
  if (isWaitingForData) {
    ...
    return (
      <Card className="py-0 min-w-0 w-full h-full flex flex-col overflow-hidden">
        <CardContent className="px-2 sm:p-6 w-full h-full flex flex-col min-h-0 flex-1">
          <div
            className={`${skeletonHeight} w-full min-w-0 flex items-center justify-center flex-1`}
          >
